...
namespace BoqiangH5
{
    public partial class UserCtrlMCU : UserControl
    {
        ...
        public void BqUpdateMcuInfo(List<byte> listRecv)
        {
            try
            {
                ...
                if(isCheckMcu)
                {
                    ...
                    {
                        CheckMCUMsgEvent?.Invoke(this, new EventArgs<Tuple<bool, string>>(new Tuple<bool, string>(false, "生产日期检查超出设置值，请检查！")));
                    }
                    isCheckMcu = false;
                    return;
                }
                ...
                if(isUpdateMCUData)
                {
                    ...
                    //UpdateMcuConfigOKEvent?.Invoke(this, EventArgs.Empty);
                    isShowMsg = false;
                }
                ...
                if (isShowMsg)
                    MessageBoxForm.Show("读取 MCU 参数成功！", "读取MCU提示",1000);
                isShowMsg = true;
        ...
        public event EventHandler<EventArgs<bool>> UpdateMcuConfigOKEvent;
