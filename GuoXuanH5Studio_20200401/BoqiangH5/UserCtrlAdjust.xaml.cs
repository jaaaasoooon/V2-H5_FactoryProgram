...
namespace BoqiangH5
{
    ...
    {        
        ...
        int readNum = 0;
        public void RequireReadRTC(int num)
        {
            isReadRTC = true;
            readNum = num;
            BqProtocol.BqInstance.BQ_ReadRTC();
        ...
        {
            if (listRecv[0] != 0xA1 || listRecv.Count < 0x60)
            {
                return;
            }
        ...
        private void BqUpdateTemp(List<byte> listRecv)
        {
            ...
            tbTemp4.Text = listRecv[67].ToString();
            tbTemp5.Text = listRecv[68].ToString();
            tbTemp6.Text = listRecv[69].ToString();
            tbTemp7.Text = listRecv[70].ToString();
            ...
            if(isAdjustRTC)
            {
                if (SelectCANWnd.m_H5Protocol == H5Protocol.BO_QIANG)
                {
                    ...
                    if (e.RecvMsg[0] == 0xB5)
                    {
                        ...
                        {
                            MessageBoxForm.Show("校准RTC成功！", "校准提示", 1000);
                            BqProtocol.BqInstance.BQ_ReadRTC();
                        }
        ...
        public event EventHandler<EventArgs<string>> ReadSecondRTCOverEvent;
                ...
                if (e.RecvMsg[0] == 0xA5)
                {
                    ...
                    if(isReadRTC)
                    {
                        if(readNum == 1)
                        {
                            readNum = 0;
                            isReadRTC = false;
                            ReadRTCOverEvent?.Invoke(this, new EventArgs<string>(string.Format("{0}/{1}/{2} {3}:{4}:{5}", "20" + year, month, day, hour, minute, second)));
                        }
                        else if(readNum == 2)
                        {
                            readNum = 0;
                            isReadRTC = false;
                            ReadSecondRTCOverEvent?.Invoke(this, new EventArgs<string>(string.Format("{0}/{1}/{2} {3}:{4}:{5}", "20" + year, month, day, hour, minute, second)));
                        }
                    }
                }
                else
